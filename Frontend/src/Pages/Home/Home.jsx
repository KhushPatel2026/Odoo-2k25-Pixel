...
const Home = () => {
      ...
      try {
        const [questionsResponse, tagsResponse] = await Promise.all([
          QuestionService.getQuestions({
            ...
            sort: sortBy,
            answered: answeredFilter !== 'all' ? answeredFilter === 'answered' : undefined
          }),
        ...
        if (tagsResponse.success) {
          setPopularTags(tagsResponse.data.tags.map(tagObj => tagObj.tag) || []);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    socket.on('newQuestion', (newQuestion) => {
      ...
      const matchesTag = selectedTag === 'all' || newQuestion.tags.includes(selectedTag.toLowerCase());
      const matchesSearch = !searchTerm || newQuestion.title.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      if (matchesTag && matchesSearch && matchesAnswered) {
        setQuestions((prevQuestions) => {
          ...
          if (currentPage === 1 && !prevQuestions.some(q => q._id === newQuestion._id)) {
            const updatedQuestions = [newQuestion, ...prevQuestions].slice(0, limit);
    ...
    socket.on('questionUpdated', (updatedQuestion) => {
      setQuestions((prevQuestions) => {
        const index = prevQuestions.findIndex(q => q._id === updatedQuestion._id);
        if (index !== -1) {
          // Check if updated question still matches filters
          const matchesTag = selectedTag === 'all' || updatedQuestion.tags.includes(selectedTag.toLowerCase());
          const matchesSearch = !searchTerm || updatedQuestion.title.toLowerCase().includes(searchTerm.toLowerCase());
          const matchesAnswered = answeredFilter === 'all' ||
            (answeredFilter === 'answered' && updatedQuestion.answerCount > 0) ||
            (answeredFilter === 'unanswered' && updatedQuestion.answerCount === 0);
          ...
          {isMenuOpen && (
            ...
            >
              <div className="space-y-3 px-4">
                <Link to="/" className="block text-[#9b87f5] font-medium py-2 px-3 rounded-lg bg-[#9b87f5]/10">Home</Link>
                      ...
                      <div className="flex flex-col sm:flex-row items-start sm:space-x-6 space-y-4 sm:space-y-0">
                        <div className="flex flex-row sm:flex-col items-center sm:items-center space-x-6 sm:space-x-0 sm:space-y-4 min-w-[80px] w-full sm:w-auto justify-between sm:justify-start">
                          <div className="flex flex-col items-center space-y-1">
                            ...
                            <span className="text-sm font-medium text-white">{question.totalUpvotes}</span>
                          </div>
                          <div className="flex flex-col items-center space-y-1">
                            <MessageCircle className="w-5 h-5 text-white/60 group-hover:text-white transition-colors duration-300" />
                            <span className="text-sm text-white/60">{question.answerCount} answers</span>
                          </div>
                          <div className="flex flex-col items-center space-y-1">
                        ...
                        <div className="flex-1">
                          ...
                          <div 
                            ...
                            dangerouslySetInnerHTML={{ __html: truncateText(question.description) }}
                          />
